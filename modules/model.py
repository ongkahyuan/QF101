...
def model(S, K, tau, sigma, r=0.1, q=0.02, N=100):
    if tau == 0:
        return max(S-K, 0), max(K-S, 0), max(S-K, 0), max(K-S, 0)
    # 1.
    deltaT = tau/N                           # delta T is the time step
    # u is the uptick (amount that stock goes up by in each time step)
    u = math.exp(sigma*math.sqrt(deltaT))
    d = 1/u                                  # d is the downtick
    # p is risk neutral probability thingy i think (used for expectation)
    ...
    for i in range(N-1, 0-1, -1):
        for j in range(i+1):
            ...
            europeanCallOptFuturePayoff = discount * \
                (p*ec[i+1][j+1]+(1-p)*ec[i+1][j])
            americanCallOptFuturePayoff = discount * \
                (p*ac[i+1][j+1]+(1-p)*ac[i+1][j])
            europeanPutOptFuturePayoff = discount * \
                (p*ep[i+1][j+1]+(1-p)*ep[i+1][j])
            americanPutOptFuturePayoff = discount * \
                (p*ap[i+1][j+1]+(1-p)*ap[i+1][j])

            currentStockPrice = S*(u**j)*(d**(i-j))

            ec[i][j] = europeanCallOptFuturePayoff
            ac[i][j] = max(americanCallOptFuturePayoff,
                           max(currentStockPrice-K, 0))
            ep[i][j] = europeanPutOptFuturePayoff
            ap[i][j] = max(americanPutOptFuturePayoff,
                           max(K-currentStockPrice, 0))

    # 4.
    euro_c = round(ec[0][0], 2)
    euro_p = round(ep[0][0], 2)
    amer_c = round(ac[0][0], 2)
    amer_p = round(ap[0][0], 2)
